...
const findGreatestProductOfAdjacent = adjacent => numbers => {
  ...
  while (count[1] < numbers.length) {
    let value = numbers.slice(...count).reduce(product)
    greatest = value > greatest ? value : greatest
    // console.log(numbers.slice(...count), value, greatest)
    count = count.map(n => n + 1)
  }
  return greatest
}

function findGreatestOfGridWith(grid, func) {
  return grid.map(func).reduce((a, b) => (b > a ? b : a), 0)
}

function udGridTransform(grid) {
  let transformedGrid = []
  let arr = []
  let i = 0
  let j = 0
